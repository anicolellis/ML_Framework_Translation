...
class myNeuralNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        for _, i in enumerate(layer_list):
            ...
            if 'locally_connected' in gParameters:
                # I am unsure if this functionality is properly supported in torch
            ...
            if gParameters['pool']:
                ...
                self.pools.append(nn.MaxPool1d(pool_list[i // 3]))
    ...
    def forward(self, x):
        ...
        if len(self.linears) == 0:
            ...
            print('linears: ', self.linears)
        
        #print(len(self.linears)-1, '=2')
        for i in range(len(self.linears)-1):
            #print('linear', self.linears[i])
            x = self.linears[i](x)
            #print('act', self.act)
            x = self.act(x)
            if gParameters['dropout']:
        ...
        #print('final linear', self.linears[-1])
        x = self.linears[-1](x)
